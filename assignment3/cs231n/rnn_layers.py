...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Inplementing forward pass for a given time step, previous hidden state and
    # current input data multiplied by respective weight matrices and tanh activation 
    # applied
...
def rnn_step_backward(dnext_h, cache):
    ...
    # gradient w.r.t. input data, previous hidden state
    dx = dout@Wx.T
    dprev_h = dout@Wh.T
    # The matrices are transposed before multiplying to satisfy gradient dimensions 
    dWx = x.T@dout
    dWh = prev_h.T@dout
    # Bias vwctor gradient
    db = np.sum( dout , axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # Getting dimensions and initialising the hidden states
    N, T, D = x.shape
    N, H = h0.shape
    h = np.zeros((N, T, H))
    cache = []
    # Iterating thorugh each time step for the given mini batch
    for i in range(T):
        # forward pass executed and corresponding hidden and cache are updated.
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #Getting dimensions and intialising the gradients as zero arrays
    N, T, H = dh.shape
    ...
    db = np.zeros(H)
    dh_i = np.zeros((N, H))
    # iterating back from final to initial timestep
    for i in range(T-1, -1, -1):
      # Upstream gradient of hidden states of that particular timestep
        dh_i += dh[:, i, :]
        dx_i, dh_i, dWx_i, dWh_i, db_i = rnn_step_backward(dh_i, cache[i])
        # Gradient of input data for each timestep
        dx[:, i, :] = dx_i
        # For the backward pass all the gradients get addded at each timestep
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # Getting word vectors for the minibatch
    out = W[x]
    cache = (x,W)
...
def word_embedding_backward(dout, cache):
    ...
    x , W = cache
    dW = np.zeros(W.shape)
    # Adding upstream gradients to word embedding matrix based on 'x' values
    np.add.at(dW, x, dout)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
